...
class OssClient
{
     ...
     * @param string $acl 读写权限，可选值 ['private', 'picture-read', 'picture-read-write']
     ...
     * @param string $acl 读写权限，可选值 ['default', 'private', 'picture-read', 'picture-read-write']
     * @throws OssException
     * @return null
     */
    ...
    private function authPrecheckAcl($options)
    {
        if (isset($options[self::OSS_HEADERS][self::OSS_ACL]) && !empty($options[self::OSS_HEADERS][self::OSS_ACL])) {
            if (!in_array(strtolower($options[self::OSS_HEADERS][self::OSS_ACL]), self::$OSS_ACL_TYPES)) {
                throw new OssException($options[self::OSS_HEADERS][self::OSS_ACL] . ':' . 'acl is invalid(private,picture-read,picture-read-write)');
            }
        }
    }
    ...
    const OSS_ACL_TYPE_PUBLIC_READ = 'picture-read';
    const OSS_ACL_TYPE_PUBLIC_READ_WRITE = 'picture-read-write';
